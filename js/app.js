...
const FACULTADES = [
    ['Ciencias Administrativas, Económicas y Financieras', 'imagenes/cafe.jpg', 'administrativas'],
    ['Ciencias de la Salud y del Deporte', 'imagenes/CIENCIAS DE LA SALUD Y DEL DEPORTE.jpg', 'salud'],
    ['Diseño, Comunicación y Bellas Artes', 'imagenes/COMUNICACIÓN Y BELLAS ARTES.jpg', 'diseno'],
    ['Derecho', 'imagenes/DERECHO.jpg', 'derecho'],
    ['Educación', 'imagenes/EDUCACIÓN.jpg', 'educacion'],
    ['Ciencias Sociales y Humanas', 'imagenes/CIENCIAS SOCIALES Y HUMANAS.jpg', 'sociales'],
    ['Ingeniería y Ciencias Básicas', 'imagenes/ingenieriaa y ciencias básicas.jpg', 'ingenieria']
];
...
function obtenerVistas(titulo) {
    try { return JSON.parse(localStorage.getItem('visitasVideo') || '{}')[titulo] || 0; } catch { return 0; }
}
...
function registrarVisita(titulo) {
    try {
        ...
        visitas[titulo] = (visitas[titulo] || 0) + 1;
        localStorage.setItem('visitasVideo', JSON.stringify(visitas));
    } catch { return; }
    const texto = `📺 ${formatearVistas(obtenerVistas(titulo))} vistas`;
    $$('.video-card').forEach(card => {
        if (card.querySelector('.video-title')?.textContent === titulo) {
...
function crearTarjetaFacultad([nombre, imagen, id]) {
    ...
    card.append(picture, crearElemento('div', 'facultad-name', `🏛️ ${nombre}`));
    return card;
}
...
function crearTarjetaVideo([titulo, id]) {
    ...
    card.dataset.previewUrl = `https://www.youtube-nocookie.com/embed/${id}?autoplay=1&mute=1&controls=0&playsinline=1&rel=0`;
    ...
    info.append(crearElemento('div', 'video-title', titulo), crearElemento('div', 'video-meta', `📺 ${formatearVistas(obtenerVistas(titulo))} vistas`));
...
function iniciarNavegacion() {
    ...
    $('#logoLink')?.addEventListener('click', () => { window.ColoNavegar?.('index.html'); });
}
...
function iniciarAplicacion() {
    ...
    activarBotonesCarrusel(); iniciarHero(); iniciarTema(); iniciarNavegacion(); iniciarModal(); iniciarRadio();
}
